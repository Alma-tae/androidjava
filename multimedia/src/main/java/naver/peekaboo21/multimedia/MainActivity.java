...
public class MainActivity extends AppCompatActivity {
    ...
    Handler messageHandler = new Handler(Looper.getMainLooper()) {
        ...
        public void handleMessage(Message msg) {
            ...
            //재생할 노래를 가져오면
            if (result == true) {
                //노래 제목을 출력하고 Seekbar(ProgressBar)의 범위를 노래의 길이로 변경
                resultMsg = "재생 준비 완료";
                fileName.setText("파일 : " + list.get(msg.what)); //msg.what 대신 idx 해도 됨
    ...
    //재생할 위치의 노래의 인덱스를 받아서 재생기에 노래를 설정해주는 메소드
        ...
        new Thread(){
            ...
            public void run(){
                try {
                    ...
                    //옵셜 설정 - 파라미터 전송 방법, 시간, 캐시 사용 여부, 파라미터 전송 처리
                    con.setConnectTimeout(30000);

                    //스트림 생성 - byte 단위인지 character 단위인지 여부에 따라 다르게 생성
                    ...
                    player.setOnCompletionListener(new MediaPlayer.OnCompletionListener() {
                        ...
                        public void onCompletion(MediaPlayer mp) {
                            //마지막 노래이면 첫번째 노래로 돌아가고, 그렇지 않으면 현재 인덱스 +!
                    ...
                    //palyer가 노래를 재생하다가 에러가 발생한 경우(MediaPlayer의 OnErrorListener가 처리)
                    player.setOnErrorListener(new MediaPlayer.OnErrorListener(){
                        @Override
                        public boolean onError(MediaPlayer mp, int whtat, int extra){
                    ...
                    //Seekbar의 thumb를 사용자가 움직였을 때, 처리하는 이벤트
                    progress.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
                        //Seekbar의 thumb를 움직이고 난 후, 호출되는 메소드
                        @Override
