...
public class MovieListActivity extends AppCompatActivity {
    ...
    //마지막 항목 뷰가 스크롤에 의해서 보여진 것인지를 판별한 프로퍼티
    ...
    class ThreadEx extends Thread{
        public void run(){
            try{
                ...
                br.close();
                con.disconnect();
                //데이터 확인 - 원래 개발이라면 주석 처리를 해주는 것이 좋음
                ...
                Log.e("다운로드 또는 파싱 예약", e.getLocalizedMessage());
            }
        }
    }
    ...
    protected void onCreate(Bundle savedInstanceState) {
       ...
       //ListView 출력을 위한 설정
        ...
        //다운로드 받고, JSON 파싱을 수행한 후 재출력을 위해서 핸들러를 호출하는 스레드를 생성하고 실행
        //이전 스레드가 동작 중이면 중지
        if(th != null){
            return;
        }
        ...
        listView.setOnScrollListener(new AbsListView.OnScrollListener() {
            //스크롤 상태가 변경될 때 호출되는 메소드 - 다음 페이지 데이터를 가져오기 수행
            @Override
            public void onScrollStateChanged(AbsListView view, int scrollState) {
                ...
                && lastItemVisibleFlag == true){
                    ...
                    //10은 페이지 당 데이터 개수
                    if(page * 10 >= count){
                        Snackbar.make(view, "더 이상 데이터가 없습니다.", Snackbar.LENGTH_LONG).show();
                        return;
                    }
                    //스레드가 동작 중이면 중지
                    if(th != null){
        ...
        swipe_layout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                ...
                if(page *10 >= count){
                    Snackbar.make(getWindow().getDecorView().getRootView(), "업데이트할 데이트가 없습니다.", Snackbar.LENGTH_LONG).show();
                    return;
                }
